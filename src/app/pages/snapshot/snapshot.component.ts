...
export class SnapshotComponent implements OnInit {
  _searchTitle: Array<any> = [
    {key: 'startTime', name: '开始时间', type: 'datetime', nzSpan: 6},
    {key: 'endTime', name: '结束时间', type: 'datetime', nzSpan: 6}
  ];
  ...
  deleteRow(data) {
    this.http.post(api.deleteSnapshot, JSON.stringify(data), {
  ...
  sendData(data) {
    if (this.isAdd) {
      this.http.post(api.addSnapshot, data, {
        headers: new HttpHeaders({
          'Content-type': 'application/json;charset=UTF-8'
        })
      }).subscribe((res) => {
      ...
      this.http.post(api.editSnapshot, data, {
        headers: new HttpHeaders({
          'Content-type': 'application/json;charset=UTF-8'
        })
      }).subscribe((res) => {
  ...
  getSnapshot() {
    this.isLoading = true;
    this.http.get(api.querySnapshot).subscribe((res) => {
      console.dir(res);
      const list = <any>res;
      this._dataSet = list;
  ...
  /**根据条件查询方法*/
  querySnapshotByConditions(data) {
    this.isLoading = true;
    console.log(data);
    console.log(parseParam(data));
    const date = {
      startTime: dateFormat(new Date(data.startTime), 'yyyy-MM-dd hh:mm:ss'),
      endTime: dateFormat(new Date(data.endTime), 'yyyy-MM-dd hh:mm:ss')
    };
    this.http.get(api.querySnapshotByConditions + parseParam(date)).subscribe((res) => {
      console.dir(res);
      const list = <any>res;
