...
export class ServerComponent implements OnInit {
   ...
   * 类型
   */
  _serverTypeOptions = serverTypeOptions;
  ...
  _titles: Array<any> = [
    ...
    // },
    {
      ...
      name: '类型',
      type: 'select',
      options: this._serverTypeOptions
    },
    // {
    //   key: 'networkFlow',
    //   name: '网络流量',
  ...
  deleteRow(data) {
    this.http.post(api.deleteServer, JSON.stringify(data), {
  ...
  sendData(data) {
    if (this.isAdd) {
      this.http.post(api.addServer, data, {
        headers: new HttpHeaders({
          'Content-type': 'application/json;charset=UTF-8'
        })
      }).subscribe((res) => {
      ...
      this.http.post(api.editServer, data, {
        headers: new HttpHeaders({
          'Content-type': 'application/json;charset=UTF-8'
        })
      }).subscribe((res) => {
